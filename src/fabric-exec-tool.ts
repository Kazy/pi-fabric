...
export const createFabricExecTool = (
  ...
  defineTool({
    ...
    description: python
      ...
      : "Execute type-checked TypeScript through Fabric's configured executor for Pi core tools, MCP, Fabric providers, discovery, and extensions. QuickJS is isolated by default; the optional Node/Bun process is an unsafe trusted-code escape hatch. In full code mode, and always in Schema enforce mode, this is the exclusive model tool path. Read with `pi.ls`/`pi.read`/`pi.grep`/`pi.find`, not with `pi.bash` running ls/cat/grep/find; `pi.bash` is for effects and for commands with no `pi.*` equivalent. Keep each call in its own variable, filter in TypeScript, and return one small object: at most 1500 characters per file or command unless the user asks for full content. `tools.list()` returns summaries without schemas; use `tools.describe({ref})` for one schema and never return a raw listing.",
    promptSnippet:
