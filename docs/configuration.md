...
`executor.typeCheck` affects **TypeScript only** (the **Type check (TS)** setting). `"lenient"` (the default) suppresses property-miss diagnostics on guest results, so `(await pi.read(p)).content` compiles and resolves to `undefined` at runtime, and unknown MCP servers or tools resolve at dispatch. `"strict"` reports those misses before execution with a line number, which rejects `.content` on a read string, `.trim()` on a bash envelope, misspelled result fields, and MCP servers or tools absent from the discovered declarations. Both modes accept `unknown` results, positional and alias call forms, `π.key`, and settled-result narrowing on `r.ok`.
